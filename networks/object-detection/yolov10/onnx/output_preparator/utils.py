...
def filter_bboxes(
    ...
    Args:
        predictions (dict): predictions computed by neural networks with shape (BATCH, N, COORDS+SCORE+NC), where
                            - N: SUM((GRID_SIZE_X * GRID_SIZE_Y * NB_CHANNELS)),
                                i.e. YOLO11(640^2) : N = 20*20 + 40*40 + 80*80 = 400 + 1600 + 6400 = 8400
                            - COORDS: XYWH (4) + NB_CLS (80 for COCO2017) = 84
    ...
    if predictions.shape[2] > predictions.shape[1]:
        prediction = numpy.transpose(predictions, (0, 2, 1))
    output = [numpy.zeros((0, 6))] * batch
    for pi, pred in enumerate(prediction):  # image index, preds (BATCH, DETECTIONS, COORDS+CLS)
        # Apply constraints by confidence
        candidates = numpy.abs(pred).max(axis=1) >= conf_thres  # candidates
        ...
        box = box[i]
        conf = pred[i, 4 + j, None]
        pred = numpy.concatenate((box, conf, j[:, None]), axis=1)
        # Check shape
        if not pred.shape[0]:  # if no boxes, continue
            continue
        # Batched NMS
        c = pred[:, 5:6] * max_wh if prediction.shape[0] > 1 else 0
        boxes  = pred[:, :4] + c  # boxes (offset by class)
        scores = pred[:, 4]  # scores
        i = nms(boxes, scores, iou_thres)[:max_det]  # NMS
        output[pi] = pred[i]
    return output
...
def process_detections(
    ...
    # Get detections as list
    if isinstance(preds, dict):
        preds = list(preds.values())

    # Apply non max suppression algorithm
    out = filter_bboxes(
        preds[0], # it supposed that YOLO return 1 output
        conf_thres,
        iou_thres,
        max_det=100,
    )
    ...
    detects = []
    for det in out:  # detections per image
        ...
        for *xyxy, conf, cls in det:
            ...
            print(f"{head}  >> [Post-proc] prediction: {conf:.4f} - "
                  f"{cls_name} - {xyxy}{reset}") if dbg else None
